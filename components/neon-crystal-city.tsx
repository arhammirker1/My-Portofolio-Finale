...
const NeonCrystalCity: FC<NeonCrystalCityProps> = ({
  ...
  useEffect(() => {
    ...
    const gl =
      (canvas.getContext("webgl2") as WebGL2RenderingContext) || (canvas.getContext("webgl") as WebGLRenderingContext)
    if (!gl) {
    ...
    const renderFrame = (
      ...
      speedLoc: WebGLUniformLocation,
      tileLoc: WebGLUniformLocation,
      unionLoc: WebGLUniformLocation,
      stepsLoc: WebGLUniformLocation,
      maxLoc: WebGLUniformLocation,
      surfLoc: WebGLUniformLocation,
  ...
  return (
    <div role="region" aria-label={ariaLabel} className={`relative w-full h-full overflow-hidden ${className}`}>
      {error && (
        <div className="absolute inset-0 bg-black/80 flex items-center justify-center text-white font-mono">
          {error}
        </div>
      )}
